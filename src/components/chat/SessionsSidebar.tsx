...
import { Plus, Trash2, MessageSquare, Pencil, PanelLeft, PanelLeftClose } from 'lucide-react';
...
export function SessionsSidebar({
  ...
  return (
    <div className="relative flex-shrink-0 flex h-full">
      ...
      {/* Sidebar panel — always mounted, visibility via transform */}
      <aside
        className={cn(
          'fixed md:relative z-40 md:z-auto h-full w-[260px] flex-shrink-0',
          'border-r border-border bg-sidebar-background',
          'transition-transform duration-200 ease-out will-change-transform',
          isOpen ? 'translate-x-0' : '-translate-x-full md:-translate-x-full'
        )}
        // On desktop when closed, collapse to 0 width in flow
        style={!isOpen ? { marginRight: '-260px' } : undefined}
      >
        <div className="flex flex-col h-full w-[260px]">
          ...
          {/* Sessions list — always mounted to preserve scroll */}
      ...
      <button
        onClick={onToggle}
        className={cn(
          'flex-shrink-0 flex items-center justify-center w-8 h-8 my-auto',

...
export const ChatMessage = memo(function ChatMessage({ message, onActionClick, onRetry }: ChatMessageProps) {
          ...
          {message.highlights && message.highlights.length > 0 && (
            <div className="flex flex-wrap gap-2 pt-2">
              {message.highlights.map((h, i) => (
                <div key={i} className="inline-flex flex-col gap-0.5 px-3 py-2 rounded-lg bg-primary/10 border border-primary/20">
                  <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">{h.label}</span>
                  <span className="text-sm font-semibold text-primary">{formatCurrency(h.value)}</span>
                </div>
              ))}
            </div>
          )}
